...
def issue_kiosk_session(cur, user_id, role) -> str:
    ...
    ЗАЧЕМ. Все действия с материалом (закрытие рулона, списание брака, возврат на
    рулон) сервер разрешает ТОЛЬКО по ключу сессии — из тела запроса права больше
    не читаются, иначе кто угодно мог бы назваться администратором.

    А вход на терминал ключа не выдавал вовсе: сотрудник подносил бейдж, работал,
    но для сервера оставался неизвестным. Отсюда «Войдите в систему заново» на
    закрытии рулона — швеи и закройщицы упирались в это каждый раз, и рулоны
    тесьмы копились незакрытыми. Ни одна швея не имела живой сессии вообще.

    Роль берём ту, которой человек работает В ЭТУ СМЕНУ: закройщица, вышедшая
    швеёй, должна получить права швеи, иначе тесьму ей не закрыть.
    """
    token = secrets.token_urlsafe(48)[:64]
    cur.execute(
        "INSERT INTO auth_sessions (token, user_id, role) VALUES (%s, %s, %s)",
        (token, int(user_id), role),
    )
    # Заодно подчищаем протухшее: терминал входит по десять раз за смену.
...
def handler(event: dict, context) -> dict:
    ...
    if method == 'GET':
        ...
        try:
            ...
            # ЯРЛЫК ЯНДЕКСА — ЭТО ГОЛЫЕ ЦИФРЫ, А У НАС НОМЕР С ПРЕФИКСОМ «YM-».
            #
            # На ярлыке Яндекса напечатан номер заказа «61866041025» (иногда с
            # позицией грузоместа: «61866041025-1»). В системе тот же заказ хранится
            # как «YM-61866041025-1»: префикс добавляем мы сами при загрузке.
            # Из-за одного этого префикса поиск не находил НИЧЕГО — упаковщица
            # сканировала ярлык и получала «заказ не найден», хотя вещь у неё
            # в руках. Ровно так зависли связки YM-61847776323 и YM-61866041025.
            #
            # Поэтому для чисто цифрового скана (и «цифры-дефис-позиция») строим
            # кандидатов с префиксом и ищем по ним тоже. В поставках эта же
            # подстановка уже сделана — здесь, на терминале, её просто не было.
            ym_clause = ''
            _ym_digits = order_number.strip()
            if re.fullmatch(r'\d{6,}', _ym_digits):
                # Отсканирован номер заказа целиком: в нём может быть несколько
                # вещей (связка). Берём по group_key — нужную выберет сортировка
                # ниже: первой идёт та, что сейчас на стикеровке.
                _ym_esc = _ym_digits.replace("'", "''")
                ym_clause = f"   OR o.group_key = 'YM-{_ym_esc}' "
            elif re.fullmatch(r'\d{6,}-\d{1,3}', _ym_digits):
                # Номер грузоместа с позицией — это конкретная вещь связки.
                _ym_esc = _ym_digits.replace("'", "''")
                ym_clause = f"   OR o.order_number = 'YM-{_ym_esc}' "
    ...
    if method == 'POST':
        ...
        try:
            ...
            if action == 'login_by_code':
                ...
                # Ключ сессии — по нему сервер узнаёт сотрудника во ВСЕХ дальнейших
                # действиях: закрытие рулона, списание брака, возврат на рулон.
                # Без него терминал отвечал «Войдите в систему заново» на каждом
                # движении материала, и рулоны в цехе не закрывались.
                #
                # Права даём по роли текущей смены: закройщица, вышедшая швеёй,
                # должна работать с тесьмой, а не с тканью.
                session_role = (s_row[4] if s_row else None) or u_row[2]
                token = issue_kiosk_session(cur, u_row[0], session_role)
                conn.commit()
                ...
                return {
                    ...
                    'headers': headers,
                    'body': json.dumps({
                        # Терминал кладёт ключ в память браузера, и дальше он сам
                        # подставляется в каждый запрос к серверу.
                        'token': token,
                        'user': {
                            'id': u_row[0],
                            'name': u_row[1],
            ...
            if action == 'repack_scan':
                ...
                #
                # На вернувшемся пакете живут два кода: наклейка возврата (её клеят на
                # ПВЗ) и ярлык отправления, с которым вещь уезжала к покупателю. Что-то
                # из них обязательно уцелело — по ним вещь и опознаём.
                #
                # СТИКЕР ХРАНЕНИЯ (GW-xxxxxx) СЮДА НЕ ВХОДИТ.
                #
                # Упаковщица его не сканирует, а ПЕЧАТАЕТ — на отказные вещи, которые
                # едут на полку. Искать по нему на входе неоткуда: на вернувшемся от
                # покупателя пакете нашего складского стикера просто нет.
                #
                # Принимаем:
                #   * наклейку возврата маркетплейса (marketplace_returns.return_barcode);
                #   * номер отправления из заявки возврата (posting_number);
                #   * ЯРЛЫК ОТПРАВЛЕНИЯ FBS всех трёх площадок: номер отправления OZON,
                #     код стикера WB (*DWto4dQG — со звёздочкой и без, плюс номер
                #     сборочного задания цифрами) и номер заказа Яндекс Маркета.
                #
                # Зачем FBS-ярлыки. Вещь вернулась по возвратному стикеру, кладовщик
                # отправил её в цех — а наклейку возврата на ПВЗ содрали, и живым на
                # пакете остался только ярлык, с которым вещь уезжала. Упаковщица пикала
                # его и получала «вещь не найдена»: сканировать было нечего, и вещь
                # несли обратно кладовщику выяснять, что это.
                #
