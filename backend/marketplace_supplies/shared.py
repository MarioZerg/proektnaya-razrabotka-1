...
def keep_fbs_label(cur, goods_id):
    """Нельзя ли снимать с этой вещи ярлык отправления и класть её на полку.

    ЗАЧЕМ ЭТО НУЖНО.

    Вещь, сшитая в цехе под FBS-заказ (receive_reason='fbs_ready'), выходит из
    упаковки уже заклеенной ярлыком покупателя. Этот ярлык — единственный
    документ отправления: OZON выдаёт его ОДИН раз, и повторно получить его
    нельзя. Как только отправление переходит в «ожидает отгрузки», очередь
    стикеровки такой заказ не берёт — печатать нечего.

    Поэтому снятие ярлыка у живой FBS-вещи — необратимая потеря. Вещь после
    этого попадает в мёртвую петлю: в поставку не сканируется («ярлык не
    наклеен»), в стикеровку не возвращается (отправление уже собрано), а
    терминал предлагает положить её на склад и выдаёт стикер GW. Заказ
    числится «готов к отгрузке» на площадке и молча висит, пока не
    просрочится.

    Здесь мы отвечаем на один вопрос: ярлык на вещи ещё действует? Если да —
    вещь остаётся отправлением и просто возвращается в очередь на отгрузку.

    Возвращает True, если вещь трогать нельзя (и тогда переводит её в
    'awaiting_supply' — ждать следующей поставки).
    """
    cur.execute(
        "SELECT ro.id FROM goods_warehouse gw "
        # Заказ, под который вещь ЕДЕТ: сшита прямо под него или подобрана с полки.
        "JOIN orders ro ON ro.id = COALESCE(gw.reserved_order_id, gw.order_id) "
        "WHERE gw.id = %s "
        "  AND COALESCE(gw.receive_reason, '') = 'fbs_ready' "
        "  AND gw.shipping_labeled_at IS NOT NULL "
        "  AND gw.shipped_at IS NULL "
        # Ярлык действует, только пока отправление живо. У отменённого и уже
        # уехавшего заказа ярлык недействителен — такую вещь честно кладём
        # на полку, для неё это правильный путь.
    ...
    if not cur.fetchone():
        return False
    # Вещь не на полке, а «на поставку»: ждёт, когда её отсканируют в короб.
    cur.execute(
...
def notify_supply_assembled(cur, supply_id, actor_name=None):
    """Сообщает менеджеру в MAX, что кладовщик собрал поставку.

    ЗАЧЕМ. Кладовщик закончил сборку и ушёл со смены, а менеджер узнаёт об этом,
    только если сам откроет список поставок. Дальше по поставке его работа:
    свести документы, передать перевозчику. Сообщение в MAX закрывает разрыв —
    он видит готовность сразу, не заходя в систему.

    Кому пишем: менеджерам с привязанным MAX, а если их нет — администраторам,
    чтобы уведомление не потерялось совсем.

    Ошибку отправки ГЛУШИМ намеренно. Поставка уже собрана и статус сменён;
    падать из-за недоступного мессенджера и откатывать работу кладовщика нельзя.
    """
    cur.execute(
        "SELECT s.supply_number, s.ozon_application_number, s.marketplace, s.type, "
        "       s.cluster, s.supply_date, "
        "       (SELECT COUNT(*) FROM marketplace_supply_items i WHERE i.supply_id = s.id), "
        "       (SELECT COUNT(*) FROM marketplace_supply_boxes b WHERE b.supply_id = s.id) "
        "FROM marketplace_supplies s WHERE s.id = %s",
        (int(supply_id),),
    )
    row = cur.fetchone()
    if not row:
        return {'sent': 0}
    number, app_number, marketplace, stype, cluster, supply_date, items, boxes = row

    cur.execute(
        "SELECT max_user_id FROM users "
        "WHERE role = 'manager' AND max_user_id IS NOT NULL AND is_active = true"
    )
    targets = [r[0] for r in cur.fetchall()]
    if not targets:
        cur.execute(
            "SELECT max_user_id FROM users "
            "WHERE role = 'admin' AND max_user_id IS NOT NULL AND is_active = true"
        )
        targets = [r[0] for r in cur.fetchall()]
    if not targets:
        return {'sent': 0}
    ...
    lines = [
        'Поставка собрана — можно отгружать',
        '',
        f'{marketplace or ""} {stype or ""} № {number or app_number or supply_id}'.strip(),
    ]
    if cluster:
        lines.append(f'Склад: {cluster}')
    if supply_date:
        lines.append(f'Дата поставки: {supply_date:%d.%m.%Y}')
    lines.append(f'Коробов: {boxes} · вещей: {items}')
    if actor_name:
        lines.append(f'Собрал: {actor_name}')
    text = '\n'.join(lines)
...
def cancelled_item_info(cur, goods_id):
    ...
    return {
        ...
        # Складская запись вещи: по ней кладовщик выбирает полку прямо в окне
        # отмены, не заходя на страницу склада.
        'goodsId': int(goods_id),
    }


def release_cancelled_item(cur, goods_id):
    """Возвращает вещь отменённого заказа в свободный остаток склада.

    ЗАЧЕМ ЭТО НУЖНО.

    Кладовщик пикает вещь в поставку, система говорит «заказ отменён» и показывает
    штрихкод хранения. Он идёт с вещью на склад — а напечатать стикер не может:
    вещь числится отгруженной (status='shipped'), а для отгруженных печать закрыта,
    и в списке склада такой вещи нет вообще. Тупик: система назвала штрихкод, но
    сделать по нему ничего нельзя.

    Так выходит потому, что статус 'shipped' проставляется всем вещам поставки
    разом при её закрытии. Если вещь физически осталась у нас (вынули из короба,
    вернули с приёмки, заказ отменили после закрытия поставки) — в базе она
    навсегда «уехала», хотя лежит на столе.

    Поэтому в момент скана честно приводим запись в соответствие с реальностью:
    вещь у кладовщика в руках, значит она снова свободный остаток. Снимаем
    отгрузку, бронь отменённого заказа и ярлык маркетплейса — по нему вещь
    больше никуда не поедет, отправление закрыто.

    Полку НЕ назначаем: её кладовщик выберет сам в окне отмены. До этого вещь
    лежит «без полки» и попадает в очередь на раскладку.

    Возвращает True, если запись действительно поправили.
    """
    cur.execute(
        "UPDATE goods_warehouse SET "
        "  status = 'in_stock', "
        # Вещь физически у нас — отметку об отгрузке снимаем, иначе склад
        # продолжит считать её уехавшей и не даст ни печати, ни полки.
        "  shipped_at = NULL, "
        # Бронь под отменённый заказ больше не действует: держать её — значит
        # не отдать вещь новому покупателю.
        "  reserved_order_id = NULL, matched_at = NULL, "
        # Ярлык отменённого отправления недействителен: на приёмке площадки по
        # нему вещь не примут. Печатается заново, когда её подберут под новый заказ.
        "  shipping_labeled_at = NULL, shipping_labeled_by = NULL, "
        "  shipping_labeled_by_name = NULL, "
        # Полку выбирает кладовщик в окне отмены — здесь вещь «на руках».
        "  shelf_id = NULL, "
