...
def find_marketplace_item(cur, offer_id, shop_sku, barcodes=None, shop_id=None):
    ...
    #
    # Без этой строки функция падала с NameError на первом же заказе: условие
    # подставлялось в три запроса ниже, но нигде не создавалось. Синхронизация
    # обрывалась до записи заказов, и заказы Яндекса просто не доходили до
    # конвейера, хотя на самом Маркете они были.
