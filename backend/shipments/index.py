    ...
    if method == 'POST':
        ...
        try:
            ...
            if action == 'approve_supply':
                ...
                # прошлое подтверждение оборвалось на полпути. Эти рулоны держат штрихкоды
                # и не дают принять приёмку повторно: администратор снова и снова получает
                # «ошибку запроса».
                #
                # Тронутый рулон не удаляем: за ним уже стоит чей-то раскрой — лучше
                # показать честную ошибку, чем молча снести материал из работы.
                cur.execute(
                ...
                # СНАЧАЛА СНОСИМ ПОРОЖДЁННЫЕ СТРОКИ, ПОТОМ РУЛОНЫ.
                #
                # Оборванная попытка оставляет за собой не только рулоны, но и строки
                # позиций: на каждый рулон сверх первого код добавляет НОВУЮ строку
                # shipment_items с number_rolls = NULL. При следующем нажатии эти строки
                # читаются как обычные позиции — и приёмка раздувается лавиной.
                #
                # Ровно так и получилось на приёмке #325: из двух позиций пакетов
                # родилось 28 лишних строк, а рулонов стало 32 вместо 9. Причём старая
                # чистка их не убирала — она сносила только рулоны, на которые никто не
                # ссылается, а на эти ссылки как раз были.
                #
                # Признак порождённой строки — number_rolls IS NULL: у настоящей позиции
                # это поле заполняет кладовщик при оформлении приёмки.
                cur.execute(
                    "DELETE FROM shipment_items "
                    "WHERE shipment_id = %s AND number_rolls IS NULL",
                    (int(shipment_id),),
                )

                # Настоящие позиции возвращаем в исходный вид: снимаем ссылку на рулон
                # и штрихкод от сорвавшейся попытки. Иначе позиция считается уже
                # принятой, а её рулон удаляется ниже — остаётся битая ссылка.
                cur.execute(
                    "UPDATE shipment_items SET roll_id = NULL, barcode = NULL "
                    "WHERE shipment_id = %s AND roll_id IS NOT NULL",
                    (int(shipment_id),),
                )

                # Теперь на рулонах прошлой попытки не висит ни одной ссылки — сносим их
                # все. Материал встанет на склад заново, уже целиком и по одному разу.
                cur.execute(
                    "DELETE FROM rolls WHERE shipment_id = %s",
                    (int(shipment_id),),
                )
                ...
                for (item_id, material_id, quantity, number_rolls, item_price, item_currency,
                    ...
                    # number_rolls мог остаться пустым у строки, порождённой прошлой
                    # оборванной попыткой. Такие строки удаляются выше, но подстраховка
                    # нужна: раньше на int(None) приёмка падала с TypeError, и админ
                    # видел безликую «ошибку запроса», не понимая, что делать.
                    number_rolls = int(number_rolls) if number_rolls else 1
            ...
            # ЛЮБАЯ ДРУГАЯ ОШИБКА ТОЖЕ ОБЯЗАНА ОТКАТЫВАТЬ ТРАНЗАКЦИЮ.
            #
            # Раньше здесь ловились только сбои базы. Но приёмка #325 упала на
            # ОБЫЧНОЙ питоновской ошибке (int(None) на пустом числе рулонов) — она
            # прошла мимо обработчика, и отката не случилось вовсе. Половина рулонов
            # осталась в базе, приёмка — в статусе «Новый», а каждое следующее
            # нажатие «Принять» добавляло к складу новую порцию материала.
            #
            # Деньги и остатки склада не должны зависеть от того, какого именно типа
            # была ошибка: не смогли принять целиком — не принимаем ничего.
            conn.rollback()
            return {
                'statusCode': 500,
                'headers': headers,
                'body': json.dumps(
                    {'error': 'Не удалось выполнить операцию — изменения отменены, '
                              'ничего не сохранилось. Попробуйте ещё раз, а если ошибка '
                              'повторится, передайте номер документа администратору'},
                    ensure_ascii=False,
                ),
            }
        finally:
