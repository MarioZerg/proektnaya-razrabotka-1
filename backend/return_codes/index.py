...
def fetch_ozon_giveout_info(cur, giveout_id, shop_id=None):
    ...
    # ЗАБРАННЫМИ ПОМЕЧАЕМ ТОЛЬКО ТЕ ВЕЩИ, КОТОРЫЕ НАЗВАЛ САМ OZON.
    #
    # Раньше здесь стоял отбор «возьми N самых старых заявок со статусом
    # „В пункте выдачи“», где N — число отсканированных коробок. Количество
    # совпадало, а вещи — нет: в очереди ПВЗ лежат сотни заявок, и наверху
    # оказывались самые давние, а не те, что реально выдали.
    #
    # 15.09 это вышло так: кладовщик привёз выдачу, система молча пометила
    # забранными 44 заявки от 25 августа и старше. Восемь настоящих пакетов
    # он отсканировал, а остальные 36 «не пикались» — их физически не было,
    # и размеры в списке не совпадали с тем, что он держал в руках. Хуже
    # того, под эти 36 система подняла со склада УЖЕ ОТГРУЖЕННЫЕ вещи и
    # вернула их в наличие, обнулив полку и дату отгрузки.
    #
    # Теперь сверяем по номеру отправления: в ответе giveout/info OZON
    # присылает список конкретных артикулов с their posting_number. Что не
    # названо площадкой — не трогаем, даже если количество сходится. Вещь,
    # которую мы не смогли сопоставить, кладовщик отсканирует сам на складе:
    # ручной скан наклейки работает всегда и остаётся главным путём.
    picked_numbers = [
        str(a.get('posting_number') or a.get('postingNumber') or '').strip()
        for a in articles
        if a.get('approved')
    ]
    picked_numbers = [n for n in picked_numbers if n]
    ...
    if picked_numbers:
        cur.execute(
            ...
            # Ключевое условие: номер отправления назвал сам OZON в этой выдаче.
            "  AND posting_number = ANY(%s)",
            (giveout_id, shop_id, shop_id, picked_numbers),
        )
...
def stock_picked_up_returns(cur, ids=None):
    ...
    for (r_id, order_id, marketplace, external_id, product_name, material,
        ...
        #
        # НО НЕ ТРОГАЕМ ВЕЩЬ, КОТОРАЯ ЛЕЖИТ В ЖИВОЙ ПОСТАВКЕ.
        #
        # 15.09 ложная приёмка подняла со склада 26 вещей из уже закрытых
        # поставок: обнулила полку, дату отгрузки и ярлык, и они снова
        # оказались «в наличии». Для склада это худший вид ошибки — товар
        # уехал к покупателю, а система предлагает его отгрузить второй раз.
        #
        # Поэтому вещь, отсканированную в поставку, оставляем как есть.
