...
# по штрихкоду.
#
# Главное требование к лимиту: в него должны ЦЕЛИКОМ помещаться все ЖИВЫЕ рулоны
# (на складе и в цехах) — их сейчас около семисот. Иначе фильтры на вкладке
# «Рулоны» считают по неполной выдаче и расходятся со складскими остатками.
# Запас взят с большим походом: закрытые рулоны идут после живых и просто
    ...
    if method == 'GET':
        ...
        try:
            ...
            cur.execute(
                ...
                #
                # СНАЧАЛА ЖИВЫЕ РУЛОНЫ, потом закрытые. Раньше сортировка шла просто
                # по дате, и лимит съедали свежие ЗАКРЫТЫЕ рулоны: из девяти «Молний»
                # на складе в выдачу попадала одна, остальные вытеснялись закрытыми
                # рулонами других материалов. На складе числилось 9, на вкладке
                # «Рулоны» — 2, и было непонятно, где потерялся материал.
                #
                # Живых рулонов всего несколько сотен — они помещаются в лимит
                # целиком, поэтому остатки на вкладке теперь сходятся со складом.
                f"ORDER BY (r.status = 'completed'), r.created_at DESC, r.id DESC "
                f"LIMIT {ROLLS_LIST_LIMIT}"
            )
            rolls = [
                {
                    'id': r[0],
    ...
    if method == 'POST':
        ...
        try:
            ...
            if action == 'close_roll':
                ...
                # списан явно — «потерялся при закрытии» не бывает.
                #
                # НО СЧЁТЧИК ВОЗВРАТОВ НЕ УМЕНЬШАЕТСЯ ПРИ РАСКРОЕ.
                #
                # packer_returned_quantity копит всё, что когда-либо вернули на рулон, и
                # после раскроя этого куска не убывает. Проверка же читала его как
                # «столько лежит свободным прямо сейчас» — и рулон, чей возврат давно
                # ушёл в заказы, не закрывался НИКОГДА. Швея видела отказ и при 9 м, и
                ...
                if initial_qty > 0:
                    # ВОЗВРАТ УПАКОВЩИЦЫ — ЭТО ТОЖЕ МЕТРЫ НА РУЛОНЕ.
                    #
                    # При перепаковке годный кусок ткани возвращают на рулон: остаток
                    # растёт, и из него потом кроят новые заказы. Формула же считала
                    # только «сколько привезли минус сколько ушло» — про возврат она не
                    # знала. У рулона 1-005190 привезли 45.5 м, вернули 9 м, в заказы
                    # ушло 51.75 м: по формуле выходило «не хватать могло максимум
                    # −6.25 м», и терминал отказывался закрывать рулон при ЛЮБОЙ цифре,
                    # даже при нуле. Швея упиралась в стену и не могла завершить работу.
                    max_possible = initial_qty + float(row[6] or 0) - used_qty
