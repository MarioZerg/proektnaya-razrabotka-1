...
# ВЕЩЬ УЖЕ ЛЕЖИТ В ЖИВОЙ ПОСТАВКЕ — ДЛЯ ПОДБОРА ЕЁ БОЛЬШЕ НЕТ.
#
# Вещь, отсканированная в короб FBO, физически уезжает на склад площадки. Для
# нас она перестаёт существовать как остаток: в коробе её уже заклеили, на OZON
# по коробу заведено грузоместо.
#
# Но в базе такая вещь остаётся записью со своим товаром и размером. Автоподбор
# и сканер подбора искали ЛЮБУЮ подходящую вещь и спокойно отдавали её под
# новый заказ FBS: заказ помечался «закрыт складом», кладовщик шёл к стеллажу,
# а вещи там нет — она в заклеенном коробе. Отправление зависало, и найти
# причину было нельзя: по системе всё «собрано».
#
# Подставляется с нужным алиасом таблицы goods_warehouse, например:
#   GOODS_IN_LIVE_SUPPLY_SQL.format(gw='gw')
#
# Завершённые и отменённые поставки не в счёт: вещь могла вернуться к нам и
# снова стать нормальным остатком.
GOODS_IN_LIVE_SUPPLY_SQL = (
    "EXISTS (SELECT 1 FROM marketplace_supply_items _msi "
    "        JOIN marketplace_supplies _ms ON _ms.id = _msi.supply_id "
    "        WHERE _msi.goods_warehouse_id = {gw}.id "
    "          AND COALESCE(_ms.status, '') NOT IN ('Выполнена', 'Отменена'))"
)
...
def try_match_orders_from_stock(cur, gw_id=None):
    ...
    # существует.
    #
    # ВЕЩЬ ИЗ КОРОБА ПОСТАВКИ СЮДА НЕ ПОПАДАЕТ.
    #
    # Вещь, отсканированная в короб FBO, физически уехала на склад площадки, но
    # в базе осталась записью с тем же товаром и размером. Автоподбор видел её
    # как подходящий остаток и закрывал ею новый заказ FBS: заказ помечался
    # «Со склада», кладовщик шёл к стеллажу — а вещь в заклеенном коробе.
    # Отправление зависало, причину по экранам было не найти.
