...
# ЧТО СЧИТАЕТСЯ ОТМЕНЁННЫМ ЗАКАЗОМ — копия условия из backend/orders/shared.py
# (облачные функции не делят между собой код, импортировать оттуда нельзя).
#
# Покупатель отменяет заказ на стороне площадки, и к нам это приезжает её статусом:
# ozon_status='cancelled', ym_status='...CANCELLED', cancelled_at. Наш собственный
# status при этом остаётся прежним — «Новый». Поэтому судить об отмене по одному
# полю status нельзя: очередь раскроя такой заказ уже прячет, а закрытие смены его
# ещё считало — и человек не мог ни завершить заказ, ни закрыть смену.
#
# Знака процента внутри НЕТ намеренно: psycopg2 принимает его за место подстановки,
# а запросы ниже идут с параметрами. strpos делает то же самое без этой ловушки.
CANCELLED_SQL = (
...
def count_orders_in_work(cur, user_id, role, session_workshop_id=None):
    ...
    ОТМЕНЁННЫЕ ЗАКАЗЫ НЕ ДЕРЖАТ СМЕНУ. Раньше отмена проверялась только по нашему
    полю status, а покупатель отменяет заказ на стороне площадки: к нам это приезжает
    как ozon_status='cancelled' / ym_status='...CANCELLED' / cancelled_at, тогда как
    наш status остаётся «Новый». Такой заказ пропадает из очереди раскроя (она
    считает отмену по всем полям сразу), но продолжал считаться «на раскрое» при
    ...
    if role == 'packer':
        ...
        cur.execute(
            ...
            # смену. Исключаем только заказы, снятые у нас вручную (status='Отменён'):
            # их из очереди убирают совсем.
    ...
    if method == 'POST':
        ...
        try:
            ...
            if action == 'open':
                ...
                # ПРАВА СЛЕДУЮТ ЗА ДОЛЖНОСТЬЮ СМЕНЫ.
                #
                # Ключ сессии терминал получает при сканировании бейджа — ДО того, как
                # человек выбрал, кем сегодня работает. Закройщица, вышедшая швеёй,
                # получила бы права закройщика и не смогла бы закрыть рулон тесьмы:
                # сервер проверяет роль по ключу, а не по тому, что нарисовано на экране.
                # Поэтому, открыв смену, переписываем роль в её живых сессиях.
                if session_role:
                    cur.execute(
                        "UPDATE auth_sessions SET role = %s "
                        "WHERE user_id = %s AND expires_at > now()",
                        (session_role, int(user_id)),
                    )
            ...
            if action == 'toggle_task':
                # ГАЛОЧКА НА ЛЮБОМ ЗАДАНИИ СМЕНЫ.
                #
                # Раньше отметить можно было только два пункта, которые система не
                # умеет проверить (отгрузка ткани, напоминание про рулоны), — всё
                # остальное закрывалось само по данным склада. На практике это
                # запирало кладовщика: например, FBS-поставка засчитывается только
                # когда отсканирована каждая вещь, а если пару штук физически
                # отгрузить нельзя — пункт висит и не даёт закрыть смену, хотя
                # рабочий день закончен.
                #
                # Теперь кладовщик закрывает любой пункт сам. Счётчик работы
                # рядом с заданием остаётся правдивым — видно, что именно не
                # доделано, — а отметка живёт только в рамках ТЕКУЩЕЙ смены:
                # завтра список посчитается заново по живым данным склада.
            ...
            if action == 'auto_close':
                ...
                # ПОШАГОВЫЙ КОНВЕЙЕР ЗАКАЗА ХРАНИМ 60 ДНЕЙ, НЕ ВЕЧНО.
                #
                # Взял в работу, раскроил, отшил, упаковал — по этим четырём
                # действиям идёт 60% всего журнала (26 тысяч строк за месяц), и
                # каждое из них ДУБЛИРУЕТ то, что уже лежит в самом заказе
                # отдельными полями: taken_at, cut_at, sewn_at, packed_at и кто
                # именно это сделал. История заказа от удаления не теряется —
                # она читается из заказа, а не отсюда.
                #
                # Журнал нужен для разбора свежих спорных ситуаций («кто вчера
                # взял этот заказ»), и админ открывает его на сегодняшний день.
                # Двух месяцев с запасом хватает на любой разбор, а зарплата и
                # выработка считаются по полям заказа и вообще сюда не смотрят.
                #
                # Всё остальное НЕ трогаем: списания, брак, утилизация, ручной
                # приём, возвраты — это следы решений о судьбе товара и денег,
                # по ним разбираются спустя месяцы.
                cur.execute(
                    "DELETE FROM audit_log "
                    "WHERE action IN ('take_order', 'cut', 'send_to_stickering', 'close_order') "
