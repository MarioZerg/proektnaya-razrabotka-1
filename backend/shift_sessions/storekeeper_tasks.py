...
def frozen_target(cur, task_key, live_count, after_cutoff):
    """Сколько штук нужно собрать по этому заданию с учётом отсечки 15:00.

    ПОЧЕМУ ЭТО ВАЖНО. Кладовщик собирает не конкретные вещи, а НУЖНОЕ
    КОЛИЧЕСТВО: вещи одного размера на полке одинаковы, годится любая. Раньше
    задание считалось по конкретным записям — и стоило взять с полки другую
    ...
    КАК РАБОТАЕТ. Ровно в 15:00 количество замораживается: «собрать N штук».
    Дальше засчитывается ЛЮБАЯ собранная вещь, а работа, пришедшая после
    отсечки, в это число не добавляется — она уйдёт в завтрашний список.

    До 15:00 заморозки нет: список живой, показываем как есть.
    Возвращает (сколько осталось собрать, зафиксировано ли число).
    """
    if not after_cutoff:
        return live_count, False

    # Число за сегодня уже зафиксировано — берём его.
    cur.execute(
        "SELECT target_count FROM storekeeper_task_cutoff "
        "WHERE task_key = %s AND snap_date = (now() + interval '3 hours')::date",
        (task_key,),
    )
    row = cur.fetchone()
    if row is None:
        # Первый запрос после 15:00 — замораживаем текущее количество.
        cur.execute(
            "INSERT INTO storekeeper_task_cutoff (task_key, snap_date, target_count) "
            "VALUES (%s, (now() + interval '3 hours')::date, %s) "
            "ON CONFLICT (task_key, snap_date) DO NOTHING",
            (task_key, int(live_count)),
        )
        return live_count, True

    target = int(row[0])
    # Осталось = живая работа, но не больше зафиксированного на 15:00.
    #
    # Живой счётчик считается по ВСЕЙ работе, без привязки к конкретным вещам:
    # собрал любую подходящую — счётчик уменьшился. Новая работа после отсечки
    # его поднимает, но min() держит планку: сверх обещанных N штук сегодня
    # не требуем, остальное уйдёт в завтрашний список.
    return max(0, min(int(live_count), target)), True


def _admin_overrides(cur, session_id):
    """Какие задания администратор закрыл вручную ЗА кладовщика в этой смене.

    Нужно для нештатных ситуаций: кладовщик не может закрыть смену из-за
    висящей работы (сбой синхронизации, битая запись), а разобраться с
    первопричиной прямо сейчас нельзя. Администратор снимает блокировку сам.

    Привязка к КОНКРЕТНОЙ СМЕНЕ, а не к кладовщику или дню: завтра откроется
    новая смена с новым id, и чек-лист посчитается заново по живым данным —
    отметка на него не подействует.
    """
    if not session_id:
        return set()
    cur.execute(
        "SELECT task_key FROM storekeeper_task_admin_overrides WHERE shift_session_id = %s",
        (int(session_id),),
    )
...
def build_tasks(cur, session_id, user_id):
    ...
    admin_closed = _admin_overrides(cur, session_id)
    ...
    #
    # ВАЖНО: ЗАДАНИЕ — ЭТО КОЛИЧЕСТВО ШТУК, А НЕ КОНКРЕТНЫЕ ВЕЩИ.
    #
    # Вещи одного размера лежат на полке вперемешку и физически не отличаются:
    # кладовщик берёт ЛЮБУЮ подходящую, а система сама переносит подбор на ту,
    # что оказалась у него в руках. Поэтому в задании важно одно число — сколько
    # штук всего нужно собрать. Расписывать «какие именно» бессмысленно и вредно:
    # человек начинает искать конкретные вещи вместо того, чтобы просто набрать
    # нужное количество.
    cur.execute(
        ...
        # ФИЛЬТРА ПО ВРЕМЕНИ ЗДЕСЬ НЕТ — И ЭТО ГЛАВНОЕ.
        #
        # Раньше стоял срез «подобрано до 15:00», привязанный к конкретным
        # записям. Кладовщик брал с полки другую такую же вещь, система
        # переставляла ей время подбора — запись выпадала из среза, и счётчик
        # прыгал. Теперь считаем ВСЮ живую работу, а отсечку применяем к
        # ЧИСЛУ ниже (frozen_target): «собрать N штук, любых подходящих».
    )
    picking_live = int(cur.fetchone()[0] or 0)
    picking_left, picking_frozen = frozen_target(
        cur, 'picking', picking_live, after_cutoff,
    )
    tasks.append({
        'key': 'picking',
    ...
    # АДМИНИСТРАТОР ЗАКРЫЛ ЗАДАНИЕ ЗА КЛАДОВЩИКА.
    #
    # Нештатная ситуация: работа висит (сбой синхронизации, битая запись), кладовщик
    # не может закрыть смену, а разобраться с первопричиной прямо сейчас нельзя.
    # Считаем задание выполненным, но СЧЁТЧИК ОСТАВЛЯЕМ настоящим — кладовщик должен
    # видеть, что физически ещё не сделано, просто это больше не держит его смену.
    for t in tasks:
        if t['key'] in admin_closed:
            t['done'] = True
            t['adminClosed'] = True

    # КЛАДОВЩИК ЗАКРЫЛ ЗАДАНИЕ САМ.
    #
    # Раньше галочку принимали только два пункта, которые система проверить не
    # умеет; остальные закрывались строго по данным склада. Это запирало
    # человека в конце дня: например, FBS-поставка засчитывается, лишь когда
    # отсканирована каждая вещь, — а если пару штук отгрузить физически нельзя,
    # пункт висит и не отпускает смену.
    #
    # Теперь кладовщик закрывает любой пункт сам. СЧЁТЧИК ОСТАВЛЯЕМ настоящим —
    # видно, сколько работы реально осталось, — и помечаем строку, чтобы в
    # журнале смены было понятно: закрыто рукой, а не выполнено по факту.
    # Отметка живёт в пределах этой смены, завтра список считается заново.
    for t in tasks:
        if t['key'] in manual and t['key'] not in MANUAL_TASKS:
            t['done'] = True
            t['selfClosed'] = True
