...
def handler(event: dict, context) -> dict:
    ...
    GET  /?search=0123-4567      - поиск заказа по номеру МИМО лимитов списка: ищет по
                                    нашему номеру, отправлению OZON, номеру WB и Яндекса.
                                    Общий список отдаёт только свежую историю (месяц) и
                                    свежие отмены, поэтому старый заказ в нём не находится
                                    вовсе — поиск идёт прямо в базу и находит любой.
                                    Отдаёт до 100 совпадений, свежие сверху
    ...
    POST /  { action: 'bulk_cancel_preview', material, marketplace? }
        - сколько FBS-заказов этого материала можно снять с конвейера, если материал
          закончился: считает только НЕТРОНУТЫЕ (этап «Новый», без раскроя, списаний
          материала и поставки). Связки Яндекса учитываются целиком: тронута хоть одна
          вещь — не снимается вся связка. Ничего не меняет, только показывает числа
    POST /  { action: 'bulk_cancel_orders', orderIds, material? }
        - снимает очередную ПОРЦИЮ заказов: сперва отмена на маркетплейсе по API
          (WB PATCH /orders/{id}/cancel, OZON /v2/posting/fbs/cancel, Яндекс
          PUT status=CANCELLED/SHOP_FAILED), и только после подтверждения площадки —
          отмена у нас. Порциями и по таймеру, чтобы не упереться в лимит времени
          функции; возвращает { done, failed, skipped, remaining } — remaining фронт
          присылает следующим вызовом. Только администратор (проверка по токену)
    ...
    POST /  { action: 'restore_order', id }
        - вернуть ошибочно снятый заказ на конвейер: этап «Новый», без исполнителя и
          цеха. Связка Яндекса возвращается целиком. Отклоняется (409), если отмену
          сделал МАРКЕТПЛЕЙС (ozon_status/ym_status): отправления на площадке больше
          нет, отгружать вещь некуда. Только администратор (проверка по токену)
