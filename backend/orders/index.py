...
#
# Активных заказов (в работе на любом этапе) сейчас около 610, закрытых («Готовые» и
# «Со склада») — около 1450, и они копятся каждый день.
#
# Лимит 800 оказался мал: закрытые заказы не помещались, и вещь, застикерованная
# вчера, на вкладке «Готовые» не находилась вообще. Кладовщик искал товар, который
# система «потеряла», хотя он лежал в контейнере и числился в поставке WB. Это не
# косметика: по такому «пропавшему» товару поставку невозможно собрать.
#
# Отдаём 2500 — с запасом на рост. Когда закрытых станет заметно больше, историю
# надо будет уносить в отдельный архивный экран с постраничной выдачей, а не
# наращивать это число: список целиком уходит на планшеты в цех.
ORDERS_LIST_LIMIT = 2500
