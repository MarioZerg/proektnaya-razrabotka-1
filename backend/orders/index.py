...
#
# Замер на боевых данных: одна строка ответа весит ~1.25 КБ. Сверх этого лимита
# идут активные заказы (~260) и отменённые (500), поэтому закрытых берём 1500 —
# всего ~2260 строк, около 2.8 МБ, запас до потолка примерно 20%.
CLOSED_ORDERS_LIMIT = 1500
...
#
# 500 — это примерно месяц отмен при нынешнем потоке. Раньше их показывалось 149
# ЗА ВСЁ ВРЕМЯ (считались только по нашему полю), так что видно станет втрое больше.
CANCELLED_ORDERS_LIMIT = 500
