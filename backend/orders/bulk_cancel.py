...
def _cancel_on_ozon(cur, cache, order):
    ...
    #
    # ПОЧЕМУ ЗДЕСЬ НЕЛЬЗЯ ИСКАТЬ СЛОВО «cancel» В ТЕКСТЕ ОШИБКИ.
    # Раньше любой ответ, где встречалось «cancel», считался успешной отменой.
    # Но OZON пишет это слово и в отказах: например «cancel_reason_id is
    # invalid» — причина отмены не подошла кабинету. Отправление при этом
    # остаётся живым, а мы помечали заказ снятым у себя. Так вещь пропадала
    # из работы: на площадке покупатель ждёт товар, у нас заказа нет, и
    # подбор со склада его тоже не видит (отменённые он пропускает).
    # Сверяем ТОЛЬКО по кодам ответа: 404 — отправления нет, 409 — уже отменено.
...
def _cancel_on_yandex(cur, cache, order):
    ...
    if status == 200:
        return True, None
    text = _error_text(status, data)
    # Только коды ответа, без поиска слова «cancel» в тексте: это слово
    # встречается и в ОТКАЗАХ площадки (например, в названии поля запроса),
    # и такой отказ раньше принимался за успешную отмену — заказ снимался
    # у нас, продолжая висеть на площадке. 404 — заказа нет, 409 — уже отменён.
    if status in (404, 409):
        return True, None
    return False, text
