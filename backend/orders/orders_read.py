...
ORDER_LIST_COLUMNS = (
    ...
    #
    # ПЕРЕДАЧА КРОЯ МЕЖДУ ЗАКАЗАМИ.
    #
    # cut_from_number — номер на БИРКЕ, которая висит на крое. Заказ отменили
    #   после раскроя, крой отдали этому заказу, а бирка осталась старая:
    #   перепечатать её некому. Швея ищет вешалку именно по этому номеру.
    # cut_given_to_order_id — крой этого (отменённого) заказа уже отдан живому,
    #   поэтому из списка «Отменённые с кроем» он уходит.
    #
    # Оба поля идут ПЕРЕД is_cancelled: оно читается как r[-1], и вставка после
