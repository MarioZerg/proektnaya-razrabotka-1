...
def handle_post(event: dict, headers: dict, dsn: str) -> dict:
    ...
    try:
        ...
        if action == 'take_stack':
            ...
            # Сколько нераскроенных заказов у человека на руках. ОТМЕНЁННЫЕ НЕ СЧИТАЕМ.
            #
            # Покупатель отменяет заказ на площадке, а наш sewing_status остаётся
            # «На раскрое». Конвейер такой заказ прячет (вкладки отсеивают отмену),
            # раскроить его закройщик физически не может — он его не видит. Но здесь
            # он продолжал считаться незакрытым и навсегда запирал кнопку «Взять стек»:
            # человек разбирал всё до конца, на экране пусто, а сервер отвечал «у вас
            # есть 1 нераскроенный заказ». Оставалось только добирать по одной вещи —
            ...
            cur.execute(
                ...
                # ПО ЗАКАЗУ УЖЕ ЕСТЬ СШИТАЯ ВЕЩЬ — В РАСКРОЙ ОН НЕ ИДЁТ.
                #
                # Заказ мог пройти конвейер целиком: вещь сшита, отстикерована и
                # лежит на складе со своим стикером GW. Если после этого он каким-то
                # образом снова оказался в «Новом» (откат статуса, ручная правка), в
                # стек его брать нельзя — цех сошьёт второй экземпляр, ткань
                # спишется повторно, а лишняя вещь останется на руках у кладовщика.
                #
                # Именно так 22.09 в стек попали 0112212799-0220-1 и 47189664-0235-1,
                # закрытые днём раньше: тюль и тесьма списались по два раза.
                #
                # Списанные вещи (lost) не в счёт: их физически нет, и заказ
                # действительно нужно сшить заново.
                "AND NOT EXISTS (SELECT 1 FROM goods_warehouse gwx "
                "                WHERE gwx.order_id = orders.id "
                "                  AND gwx.status <> 'lost') "
        ...
        if action == 'update_order':
            ...
            if 'sewingStatus' in body_data:
                ...
                # ВЕЩЬ ПО ЗАКАЗУ УЖЕ СШИТА И ЛЕЖИТ НА СКЛАДЕ — НАЗАД НЕ ОТМАТЫВАЕМ.
                #
                # Заказ прошёл конвейер, упаковщица закрыла его на стикеровке, и на
                # складе появилась готовая вещь со своим стикером GW. Откатить такой
                # заказ в «Новый» означает отправить его по конвейеру ВТОРОЙ раз:
                # ткань спишется повторно, закройщику и швее начислится за ту же
                # работу, а в цехе появится второй экземпляр вещи, которую никто не
                # заказывал.
                #
                # Ровно так вышло с заказами 0112212799-0220-1 и 47189664-0235-1:
                # они прошли раскрой→пошив→стикеровку 21.09, а 22.09 снова попали в
                # стек закройщицы. Тюль и тесьма списались по два раза, а обе вещи
                # остались на руках у кладовщика — деть их было некуда.
                #
                # Вещь на складе — это физический факт, и отменить его правкой
                # статуса нельзя. Нужно разобраться с самой вещью: отправить её на
                # склад хранения или списать, и только потом решать судьбу заказа.
                if (
                    target_sewing in STATUS_ORDER
                    and current_sewing in STATUS_ORDER
                    and STATUS_ORDER.index(target_sewing) < STATUS_ORDER.index(current_sewing)
                ):
                    cur.execute(
                        "SELECT storage_barcode, status FROM goods_warehouse "
                        "WHERE order_id = %s AND status <> 'lost' "
                    ...
                    if gw_done:
                        return {
                            ...
                            'body': json.dumps({
                                'error': f'По заказу уже сшита вещь {gw_done[0]} — она лежит '
                                         f'на складе. Вернуть заказ в «{target_sewing}» нельзя: '
                                         f'он пойдёт по цеху второй раз, ткань спишется повторно, '
                                         f'а в цехе появится лишняя вещь. Сначала разберитесь '
                                         f'с вещью {gw_done[0]} на складе',
                            }, ensure_ascii=False),
                        }
        ...
        if action in ('cut', 'cut_group'):
            ...
            for item_id in cut_queue:
                ...
                # ПОСЛЕДНИЙ РУБЕЖ: ПО ЗАКАЗУ УЖЕ ЕСТЬ СШИТАЯ ВЕЩЬ.
                #
                # Статус «На раскрое» сам по себе ничего не гарантирует: заказ мог
                # пройти конвейер целиком, а потом вернуться в очередь откатом
                # статуса и снова попасть в стек. Ткань тогда списывается второй раз,
                # зарплата начисляется второй раз, а в цехе появляется лишняя вещь —
                # так вышло с 0112212799-0220-1 и 47189664-0235-1.
                #
                # Очередь раскроя такие заказы уже не выдаёт, но проверку дублируем
                # здесь: между взятием стека и нажатием «Раскроено» проходит время, и
                # вещь на складе может появиться за этот промежуток.
                cur.execute(
                        "SELECT storage_barcode FROM goods_warehouse "
                        "WHERE order_id = %s AND status <> 'lost' LIMIT 1",
                        (int(item_id),),
                )
                gw_exists = cur.fetchone()
                if gw_exists:
                        conn.rollback()
                        return {
                                'statusCode': 409,
                                'headers': headers,
                                'body': json.dumps({
                                        'error': f'По этому заказу уже сшита вещь {gw_exists[0]} — '
                                                 f'она лежит на складе. Раскраивать второй раз не нужно: '
                                                 f'сообщите администратору, заказ попал в очередь по ошибке',
                                }, ensure_ascii=False),
                        }
        ...
        if action == 'take_order':
            ...
            # ОТМЕНЁННОЕ ДО РАСКРОЯ — НЕ БЕРЁМ, ОТМЕНЁННОЕ ПОСЛЕ — ДОШИВАЕМ.
            #
            # Пока ткань цела, отменённую вещь шить незачем: её не отгрузят, а время
            # и тесьма уйдут. Но если крой уже сделан (cut_at заполнен), ткань
            # разрезана по размеру этого заказа и обратно в рулон не вернётся.
            # Единственный осмысленный путь — дошить вещь и положить её на склад
            # хранения: терминал стикеровки видит отмену и вместо ярлыка
            # маркетплейса выдаёт складской стикер GW.
            #
            # Раньше здесь стояло безусловное NOT(отмена), и раскроенная вещь,
            # отменённая покупателем, выпадала из очереди совсем: крой висел на
            # вешалке, а заказ «пропадал из системы».
            #
            # САМ СТАТУС «РАСКРОЕНО» — УЖЕ ДОКАЗАТЕЛЬСТВО КРОЯ, ДАЖЕ БЕЗ cut_at.
            #
            # Проверять только cut_at оказалось мало. Закройщица режет ткань, а
            # отметить раскрой не успевает: заказ отменяют раньше, и кнопка
            # «Раскроено» на нём блокируется. Статус при этом может оказаться
            # «Раскроено» (его проставил админ, разбирая завал), а cut_at пустым —
            # и вещь снова выпадала из очереди, хотя физически висит на вешалке.
            # Ровно так зависли заказы 0149246251-0168-1 и 40863907-0422-1.
            #
            # Очередь и так отбирает только sewing_status = 'Раскроено' (условие
            # выше), а это значит: вещь прошла этап раскроя и существует в цехе.
            # Поэтому отменённые здесь берём без оглядки на cut_at — их всё равно
            # надо дошить и увезти на склад по стикеру GW.
            #
            # Отменённые, не дошедшие до раскроя, сюда не попадут: у них статус
            # «Новый» или «На раскрое», и до этого условия дело не доходит.
            ...
            order_parts = []
            # ОТМЕНЁННЫЙ КРОЙ ВЫДАЁМ ПОСЛЕДНИМ — ЖИВЫЕ ЗАКАЗЫ ВАЖНЕЕ.
            #
            # Такая вещь никуда не едет: её дошивают и кладут на склад хранения,
            # срока отгрузки у неё нет. А живой заказ ждёт покупатель, и у него
            # горят сроки маркетплейса.
            #
            # Условие стоит ПЕРВЫМ в сортировке, то есть сильнее всех остальных
            # правил (FBS, кластер, приоритет площадки): отменённая вещь уйдёт
            # швее, только когда живой работы в цехе не осталось. Так крой не
            # потеряется, но и очередь собой не займёт.
            order_parts.append(f"({cancelled_sql('')}) ASC")
        ...
        if action == 'cancel_order':
            ...
            # ОТМЕНЁННЫЙ ЗАКАЗ ОБРАТНО В ОБЩУЮ ОЧЕРЕДЬ НЕ ВОЗВРАЩАЕМ.
            #
            # Конвейер не показывает отменённые заказы, которые лежат в «Новом» и
            # ни за кем не закреплены — иначе закройщики резали бы ткань на вещи,
            # которых никто не ждёт. Но если такой заказ УЖЕ у человека на руках и
            # он сбрасывает его в «Новый», вещь исчезает для всех: в очереди её не
            # видно, взять в работу некому, а крой (или вешалка) остаётся в цехе.
            # Так завис заказ 40863907-0422-1 — висел на вешалке, недоступный никому.
            #
            # Правило простое: отменённую вещь, которая уже пошла по конвейеру,
            # доводят до конца. На стикеровке она получает складской стикер GW и
            # уезжает на полку хранения вместо отправки покупателю.
            if buyer_cancelled:
                return {
                    'statusCode': 409,
                    'headers': headers,
                    'body': json.dumps({
                        'error': 'Заказ отменён покупателем — вернуть его в очередь нельзя: '
                                 'там его никто не сможет взять. Доведите вещь до конца — '
                                 'на стикеровке она получит складской стикер и уедет на полку.',
                    }, ensure_ascii=False),
                }
