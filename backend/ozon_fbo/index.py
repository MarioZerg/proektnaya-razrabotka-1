...
def load_item_lookup(cur):
    ...
    по ozon_sku и по sku(offer_id) -> (material, width, height, name, barcode, item_id)."""
    cur.execute("SELECT ozon_sku, sku, material, width, height, name, barcode, id FROM marketplace_items")
    by_ozon_sku = {}
    by_offer = {}
    for r in cur.fetchall():
        val = (r[2], r[3], r[4], r[5], r[6], r[7])
        if r[0]:
...
def handle_import_composition(cur, conn, client_id, api_key, body_data):
    ...
    for it in items:
        ...
        if not found:
            ...
            continue
        material, width, height, item_name, barcode, item_id = found
    ...
    if rows:
        result = execute_values(
            ...
            "quantity, source, material, width, height, cluster, product_barcode, marketplace_item_id) VALUES %s "
            "ON CONFLICT (order_number) DO NOTHING RETURNING id",
            rows,
            template="(%s, 'OZON', 'FBO', 'Новый', %s, 1, 'api', %s, %s, %s, %s, %s, %s)",
            fetch=True,
        )
        created = len(result)
