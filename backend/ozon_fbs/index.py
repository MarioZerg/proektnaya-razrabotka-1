...
# висят в «ожидает сборки». Берём по 25: одиночные отправления обрабатываются быстро
# (вся тяжесть — в делении многотоварных, а его ограничивает отдельный лимит ниже).
OZON_SYNC_PAGE = 25
...
def handle_sync_orders(cur, conn, client_id, api_key, actor_id, actor_name):
    ...
    for _ in range(OZON_SYNC_MAX_PAGES):
        ...
        # ПУСТАЯ страница = указатель убежал за конец очереди. Такое случается всегда,
        # когда заказы собрали: список «ожидает сборки» стал короче, а указатель остался
        # где был. Дальше синхронизация каждые 15 минут запрашивала позицию, которой
        # больше нет, получала пустоту — и НОВЫЕ заказы, стоящие в начале списка, не
        # загружались вообще. Так в системе и не хватало сотни заказов.
        #
        # Начинаем сначала: следующий запуск пойдёт с нулевой позиции и заберёт всё,
        # что накопилось. Повторы не страшны — они отсекаются по номеру заказа.
        if not page and offset > 0:
            cur.execute(
                "INSERT INTO system_settings (key, value) VALUES ('ozon_sync_offset', '0') "
            ...
            return _resp(200, {
                ...
                'postponedSplit': [],
                'unmatched': [],
                'note': 'Дошли до конца очереди OZON — следующий запуск начнёт сначала.',
            })
