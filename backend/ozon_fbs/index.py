...
#
# Берём по 50, а не по 100.
#
# Страница на 100 отправлений вместе с реквизитами юрлиц оказалась слишком
# тяжёлой: OZON готовил её дольше, чем живёт вся функция, и та обрывалась по
# таймауту, не сохранив НИЧЕГО. Именно так 28 августа загрузка встала на
# полтора часа — планировщик исправно звонил каждые 15 минут, а каждый вызов
# умирал на ожидании ответа.
#
# Полсотни OZON отдаёт быстро. Список идёт свежими вперёд, и почти вся страница —
# уже загруженные заказы: они отсекаются одной проверкой по номеру. Тяжёлая
# работа приходится лишь на новые, а их за 15 минут появляется единицы.
OZON_SYNC_PAGE = 50
...
#
# 3.2 из пяти секунд — с расчётом на ХОЛОДНЫЙ СТАРТ.
#
# Планировщик приходит раз в 15 минут, за это время функция успевает остыть,
# и КАЖДЫЙ его вызов — холодный. Платформа тратит на подъём функции 0.3-0.9
# секунды ДО того, как начинается наш отсчёт: этого времени в бюджете не видно,
# но лимит в 5 секунд оно съедает. При дедлайне 3.9 первый вызов после паузы
# стабильно обрывался, а он-то и есть единственный, который делает планировщик.
#
# Недобранное не теряется: заказы висят в «ожидает сборки», и следующий запуск
# их заберёт.
OZON_RUN_DEADLINE_SEC = 3.2
...
OZON_SCAN_BUDGET_SEC = 2.0

# Сколько ждём ответа OZON на один запрос.
#
# Раньше стояло 20 секунд — вчетверо больше, чем живёт сама функция. Медленный
# ответ площадки съедал весь запуск, платформа обрывала вызов (Code: 504), и
# набранные заказы не сохранялись. Ждать дольше, чем живёт функция, бессмысленно:
# ответ всё равно некуда будет записать.
#
# 2.5 секунды: если OZON не ответил и за это время, ждать дальше бессмысленно —
# лучше выйти, сохранив набранное, и попробовать через 15 минут.
OZON_HTTP_TIMEOUT_SEC = 2.5

# До какой секунды запуска ещё можно позволить себе сверку с OZON.
#
# Сверка стоит около секунды (отдельный запрос к площадке). Если основная
# работа уже съела почти весь бюджет, сверку пропускаем — иначе вызов
# оборвётся и не сохранятся даже загруженные заказы.
OZON_VERIFY_DEADLINE_SEC = 2.0
...
def ozon_post(path, client_id, api_key, payload, timeout=None):
    ...
    ТАЙМАУТ ОГРАНИЧЕН ОСТАТКОМ ВРЕМЕНИ ФУНКЦИИ.

    Раньше здесь стояло 20 секунд — вчетверо больше, чем живёт сама функция
    (у неё 5 секунд). Стоило OZON задуматься, и запрос продолжал ждать ответа,
    пока платформа не обрывала весь вызов по таймауту. В логах — Code: 504,
    а всё, что загрузка успела набрать, пропадало: заказы не приезжали в цех.
    Именно из-за этого 28 августа загрузка OZON встала на полтора часа, хотя
    планировщик исправно дёргал её каждые 15 минут.

    Теперь ждём ответа ровно столько, сколько осталось до дедлайна запуска.
    Не успел OZON — выходим сами, сохраняем набранное и дожидаемся следующего
    запуска через 15 минут.
    """
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout or OZON_HTTP_TIMEOUT_SEC) as r:
...
def handle_sync_orders(cur, conn, client_id, api_key, actor_id, actor_name,
    ...
    if only_numbers:
        for number in only_numbers[:OZON_CREATE_PER_RUN]:
            # Каждый номер — отдельный поход к OZON. Без оглядки на часы десяток
            # медленных ответов обрывал весь вызов, и не сохранялось ничего.
            ...
            sc, d = ozon_post('/v3/posting/fbs/get', client_id, api_key,
                              {'posting_number': number,
                               'with': {'legal_info': True}},
                              timeout=min(OZON_HTTP_TIMEOUT_SEC, left))
            if sc != 200:
                continue
    ...
    for _ in range(0 if only_numbers else OZON_SCAN_PAGES):
        ...
        # Ждём ответа ровно столько, сколько осталось до общего дедлайна: иначе
        # медленный OZON утащит за собой весь запуск, и набранное не сохранится.
        left = OZON_RUN_DEADLINE_SEC - (time.monotonic() - run_started)
        if left < 0.5:
            break
        ...
        # ВТОРАЯ ПОПЫТКА НА ПЕРВОЙ СТРАНИЦЕ.
        #
        # Первый запрос к OZON после паузы нередко идёт медленно: соединение
        # устанавливается заново. Планировщик приходит раз в 15 минут, то есть
        # КАЖДЫЙ его вызов такой — и загрузка сдавалась, не забрав ни одного
        # заказа. Второй запрос по уже открытому соединению проходит быстро,
        # поэтому даём ровно одну попытку, если время ещё есть.
        if status_code != 200 and not postings:
            left = OZON_RUN_DEADLINE_SEC - (time.monotonic() - run_started)
            if left > 1.0:
                status_code, data = ozon_post(
                    '/v3/posting/fbs/unfulfilled/list', client_id, api_key,
                    payload, timeout=min(OZON_HTTP_TIMEOUT_SEC, left),
                )
        ...
        if status_code != 200:
            ...
            #
            # OZON нередко думает над страницей дольше, чем мы можем ждать.
            # Раньше любая такая заминка обрывала весь запуск с ошибкой, и
            # заказы, уже разобранные на предыдущих страницах, пропадали —
            # загрузка вставала до тех пор, пока площадка не ответит быстро.
            #
            # Теперь прекращаем листать, но идём дальше: сохраняем то, что
    ...
    # брать больше нечего.
    chain = int(body_data.get('chain') or 0)
    # ЦЕПОЧКА ПРОДОЛЖАЕТСЯ, ТОЛЬКО ЕСЛИ РАБОТА РЕАЛЬНО ЕСТЬ.
    #
    # Раньше условием было «не дошли до конца ленты» (not reached_end). Лента
    # unfulfilled — это ВСЕ неотгруженные отправления, сотни штук, и обойти её за
    # 2.5 секунды листания невозможно НИКОГДА. Поэтому reached_end почти всегда
    # оставался False, и цепочка на 12 шагов раскручивалась при каждом запуске
    # планировщика — даже когда новых заказов не было вовсе.
    #
    # В сутки это давало 406 запусков вместо 96 по расписанию: четыре пятых из них
    # перебирали давно загруженные отправления и не создавали ничего.
    #
    # Теперь продолжаем, только когда на этом шаге что-то сделали: набрали полную
    # порцию заказов (значит, за ней есть ещё) или остались неразделённые. Пусто —
    # работа кончилась, ждём следующего запуска планировщика.
    has_more = (
    ...
    if only_numbers and chain < MAX_CHAIN and created > 0:
        ...
        #
        # НО ТОЛЬКО ЕСЛИ ОСТАЛОСЬ ВРЕМЯ.
        #
        # Сверка — это ещё один поход к OZON, почти секунда. Вместе с ответом на
        # список она не помещалась в пять секунд, отведённых функции: вызов
        # обрывался по таймауту, и заказы, уже разобранные выше, не сохранялись.
        # Загрузка вставала на часы, хотя планировщик исправно звонил каждые
        # 15 минут.
        #
...
def handle_refresh_all(cur, conn, client_id, api_key, body_data=None, actor_id=None, actor_name=None):
    ...
    #
    # СТАРЫЕ ЗАВЕРШЁННЫЕ ОТПРАВЛЕНИЯ НЕ СПРАШИВАЕМ.
    #
    # «Доставлено», «отменено» и «не принят на сортировке» — конечные состояния:
    # дальше двигаться некуда. Раньше в список попадали ВСЕ отправления за всю
    # историю (почти пять тысяч, из них больше половины давно закрыты), и обход
    # никогда не мог закончиться досрочно: выход по «нашли всё» не срабатывал,
    # потому что найти всё было невозможно — старые заказы OZON в свежем окне уже
    # не отдаёт. Функция каждый раз молотила три страницы по тысяче отправлений
    # и упиралась в свой лимит времени.
    #
    # Недельный запас оставляем намеренно: заказ, доставленный вчера, ещё может
    # уехать в возврат, и такое изменение статуса нам увидеть важно. А вот у
    # доставленного две недели назад статус уже не поменяется.
    cur.execute(
