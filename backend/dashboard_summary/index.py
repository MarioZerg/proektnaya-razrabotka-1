    ...
    try:
        ...
        if warehouse:
            ...
            parts += [
                ...
                #
                # СЧИТАЕМ РОВНО ТО ЖЕ, ЧТО ПЛИТКА «РАЗЛОЖИТЬ ПО ПОЛКАМ» НА СКЛАДЕ.
                #
                # Раньше сюда попадали ещё и возвраты с ПВЗ (mp_return), и вещи без
                # напечатанного стикера. Выходило 11 против 4 на самой странице:
                # кладовщик шёл в цех за одиннадцатью вещами, а находил четыре.
                # Возвраты — отдельная работа, у них своя плитка ниже, а вещь без
                # стикера ещё у упаковщицы: принтер мог не сработать, и звать за ней
                # в цех рано.
                "COUNT(*) FILTER (WHERE status = 'awaiting_shelf' "
                "  AND storage_labeled_at IS NOT NULL) AS awaiting_shelf, "
                ...
                #
                # Раньше считались заявки (marketplace_returns со статусом picked_up), и
                # дашборд расходился с самим складом: 5 против 7. Разницу давали вещи,
                # у которых заявки нет вовсе, — например отменённые уже после стикеровки
                # (receive_reason='cancelled_labeled'): они лежат в тех же возвратах и
                # ждут разбора, но заявкой с площадки не оформлены.
                #
                # Кладовщик открывает разбор возвратов и видит там СПИСОК ВЕЩЕЙ. Значит и
                # считать надо вещи — тем же условием, что отбирает плитка «Разобрать
                # возвраты» на складе: статус mp_return (привезли, лежит у кладовщика) и
                # checking (уже разбирает). Тогда оба числа сходятся всегда.
                "returns_part AS (SELECT COUNT(*) AS picked_up "
                "FROM goods_warehouse gw "
                "WHERE gw.status IN ('mp_return', 'checking'))",
            ]
            select_cols += [
