...
JOBS = [
    ...
    {
        'key': 'manager_accrue',
        'title': 'Начисления менеджеру',
        'func': 'manager_finance',
        'urlAction': 'accrue',
        'purpose': 'Считает процент по недельным отчётам и закрывает холды '
                   'через 15 дней',
        'group': 'service',
        # Раз в сутки: отчёты закрываются раз в неделю, а холды — по дате.
        # Чаще смысла нет, реже — начисление задержится на день.
        'everyMin': 1440,
        'lateAfter': 2880,
        'optional': True,
    },
    ...
    },
    {
        ...
        'group': 'service',
        # Раз в два часа: продажи нужны свежими, а не вчерашними. Отчёт за
        # день небольшой — площадка отдаёт его быстро.
        'everyMin': 120,
        'lateAfter': 480,
        'optional': True,
    },
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        for job in JOBS:
            ...
            # ФАКТИЧЕСКАЯ частота против ожидаемой.
            #
            # Расписание задаётся во внешнем планировщике, и система его не видит —
            # знает только, сколько раз задание пришло. Если в планировщике ошиблись
            # (поставили «раз в минуту» вместо «раз в час»), задание молча работает в
            # 60 раз чаще: счёт за облако растёт, а база начинает отвечать отказами.
            # Поэтому считаем реальный интервал по числу запусков за сутки и сравниваем
            # с тем, что заложено в задании.
            runs = a.get('perDay', 0)
            fact_every_min = round(1440 / runs, 1) if runs else None

            # «Слишком часто» — это заметно чаще задуманного, а не мелкое отклонение:
            # планировщик всегда чуть плавает, и придираться к 10-20% нельзя, иначе
            # предупреждение станет фоновым шумом. Порог — вдвое чаще нормы.
            #
            # Редкие задания (раз в 6 часов и реже) из проверки исключаем: они пишут
            # запись, только когда действительно что-то сделали, и «лишние» запуски
            # у них — это не частый вызов, а несколько срабатываний подряд. Считать
            # их перерасходом значит показывать админу ложную тревогу на 60 вызовов
            # в месяц, пока рядом стоит настоящая на сорок тысяч.
            too_often = bool(
                runs
                and fact_every_min
                and job['everyMin'] <= 60
                and fact_every_min * 2 < job['everyMin']
            )
            # Сколько лишних запусков это даёт в месяц — в этом язык денег, а не техники.
            extra_per_month = (
                int((runs - 1440 / job['everyMin']) * 30) if too_often else 0
            )
            ...
            items.append({
                ...
                'runsPerDay': runs,
                # Реально получается раз в столько минут.
                'factEveryMin': fact_every_min,
                'tooOften': too_often,
                'extraPerMonth': extra_per_month,
        ...
        return _resp(200, {
            ...
            # Сколько заданий бьют чаще нормы и во сколько это обходится за месяц.
            # Выносим в шапку: перерасход не виден в отдельной карточке — он виден
            # только суммой.
            'tooOftenCount': sum(1 for i in items if i['tooOften']),
            'extraPerMonthTotal': sum(i['extraPerMonth'] for i in items),
        })
    finally:
        conn.close()