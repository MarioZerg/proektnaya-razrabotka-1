...
def handler(event: dict, context) -> dict:
    ...
    площадку напрямую и кладём рядом свою запись и снимок из истории —
    ...
    try:
        ...
        for it in (data.get('items') or []):
            ...
            out.append({
                'offerId': offer,
                # «Ваша цена» в кабинете — её и двигает робот.
                'ozonPrice': p.get('price'),
                'ozonOldPrice': p.get('old_price'),
                # Что реально платит покупатель после скидки площадки.
                'ozonBuyerPrice': (p.get('marketing_seller_price')
                                   or p.get('marketing_price')),
