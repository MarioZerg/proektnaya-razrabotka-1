	...
	theme: {
		...
		extend: {
			...
			keyframes: {
				...
				//
				// ПРОЦЕНТЫ ЗДЕСЬ — ДОЛИ ШИРИНЫ САМОГО БЛИКА, А НЕ ПЛИТКИ.
				// Блик занимает треть карточки, поэтому один её экран — это 300%
				// его собственной ширины. Раньше стояло 220%: блик уходил, не
				// доехав до правого края, и казалось, что он глохнет на середине.
				//
				// Полный проход: от -100% (полностью слева за краем) до 300%
				// (полностью справа за краем) — светлая полоса пересекает плитку
				// целиком. Дальше держим её за краем до конца цикла: получается
				// пауза между проходами, а не безостановочное мельтешение.
				'tile-sheen': {
