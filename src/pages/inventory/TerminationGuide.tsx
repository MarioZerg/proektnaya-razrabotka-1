...
const steps: Step[] = [
  {
    ...
    text: 'Сообщите руководителю о намерении завершить сотрудничество не позднее чем за 14 календарных дней. Это условие прописано в договоре: за две недели мы успеваем перераспределить заказы и проверить качество уже сданных работ.',
    image: IMG_NOTICE,
  },
  {
    num: 2,
...
const specialCases = [
  {
    ...
    text: 'Исполнитель перестал выходить на смены, не сообщив о завершении работы, либо скрыл намерение уйти, дожидаясь ближайшей выплаты.',
  },
  {
    icon: 'TriangleAlert',
...
const faq = [
  {
    ...
    a: 'Изделие проходит долгий путь: пошив, упаковка, отгрузка, доставка покупателю. Брак вскрывается уже после того, как заказ уехал, — иногда через неделю и позже. Две недели нужны, чтобы успеть проверить сданные работы.',
  },
  {
            ...
            <CardContent className="flex flex-col gap-4 py-5 sm:flex-row">
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Расчёт проводится по частям: каждые 3 дня перечисляется 10% от накопленной
