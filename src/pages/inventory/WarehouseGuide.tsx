...
const returnSteps: Step[] = [
  ...
  },
  {
    ...
    text: 'После сканирования появятся три кнопки. Выберите одну — вещь уйдёт по нужному пути, а система сама всё запишет.',
    where: 'Отгрузки → Получение возвратов',
    path: '/crm/shipments/receive-returns',
  },
  {
    ...
    text: 'Вещи, которые вы отправили «На полку», ждут размещения. Нажмите «Разложить по полкам», отсканируйте стикер хранения и укажите полку. Только после этого товар считается проверенным и попадает в подбор заказов.',
...
const decisions = [
  {
    ...
    text: 'Вещь целая, упаковка в порядке. Система напечатает стикер хранения — наклейте его и положите вещь на полку.',
  },
  {
    icon: 'Wrench',
