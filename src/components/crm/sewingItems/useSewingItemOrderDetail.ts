...
export const useSewingItemOrderDetail = ({
   ...
   * её возьмёт другой человек. Здесь заказ уходит совсем — сервер отменяет его по
   * API маркетплейса и только после подтверждения площадки помечает отменённым у
   * нас. В истории он остаётся: вкладка «Заказы», фильтр «Отменённые».
   *
   * И право админа, и «заказ ещё не тронут» проверяет сервер по токену сессии, а не
   * эта кнопка: кнопку можно не нажимать, а запрос отправить из консоли браузера.
   */
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      const extra =
        res.cancelledIds?.length > 1
          ? ` Связка Яндекса снята целиком: вещей ${res.cancelledIds.length}.`
          : '';
      toast({
        title: 'Заказ снят с конвейера',
        description: res.note
          ? `${res.note}.${extra}`
          : `Отменён на маркетплейсе. Ищите его во вкладке «Заказы», фильтр «Отменённые».${extra}`,
      });
      setDialogOpen(false);
      load();
    } catch (e) {
      // Причину называет сервер: «заказ уже раскроен», «маркетплейс не принял
      // отмену» — по ней сразу понятно, снялся заказ на площадке или нет.
      toast({
        title: 'Заказ не снят',
        description: e instanceof Error ? e.message : undefined,
        variant: 'destructive',
