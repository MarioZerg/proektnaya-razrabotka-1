...
export const useSewingItemsData = () => {
  ...
  const load = () => {
    ...
    // ограничен по объёму и раньше делился на всех: место занимали чужие заказы,
  ...
  // Роль в зависимостях нужна целиком, а не одним признаком «производственник»:
  // у совместителя (швея + закройщик) от неё зависит, ЧЬЮ историю просить у
  // сервера. Без этого при переключении роли список остался бы от прежней.
  useEffect(() => {
    load();
