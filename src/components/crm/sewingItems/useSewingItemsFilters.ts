...
export const useSewingItemsFilters = ({
  ...
  //
  // НО ЕСЛИ КРОЙ УЖЕ СДЕЛАН — ПРЯТАТЬ НЕЛЬЗЯ. Ткань разрезана, вернуть её в рулон
  // невозможно, и единственный осмысленный путь — дошить вещь и положить её на
  // склад хранения под стикером GW (его выдаёт терминал стикеровки). Раньше такая
  // вещь исчезала со всех вкладок: крой висел на вешалке, швея его взять не могла
  // (очередь прячет отменённые), и заказ «пропадал из системы» — ровно та жалоба,
  // с которой пришли по заказу 0160542020-0189-1.
  //
  // Признак «крой сделан» — cutAt: он проставляется в момент раскроя и никогда
  // не сбрасывается. До раскроя отмена по-прежнему убирает вещь из очереди.
  // Разбор самих отмен — в «Заказах», фильтр «Отменённые».
  const activeOrders = orders.filter((o) => !isOrderCancelled(o) || !!o.cutAt);
