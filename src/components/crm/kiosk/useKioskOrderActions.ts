    ...
    try {
      ...
      if (order.orderType === 'FBS') {
        ...
        if (mp === 'WB') {
          ...
          // СВЯЗКА ЯНДЕКСА: ярлык отправления ОДИН на весь заказ покупателя.
          //
          // Раньше ярлык печатался только у ПОСЛЕДНЕЙ вещи связки, а на остальных
          // терминал показывал «ярлык не нужен». Это ломало работу: кнопка «Закрыть
          // заказ» появляется только после печати, поэтому первую вещь связки
          // закрыть было нельзя — она навсегда висла на стикеровке. Упаковщица
          // видела «товар уже застикерован» и не могла довести заказ до конца.
          //
          // Теперь ярлык доступен на КАЖДОЙ вещи: связку пакуют в одну коробку, и
          // упаковщице нужен он в руках, чтобы наклеить на неё. Лишние копии просто
          // не клеятся — на коробке остаётся один ярлык, как и требует площадка.
          await printLabelPdf(await fetchYandexLabel(order.orderNumber), 'Ярлык Яндекс Маркета');
          if ((order.groupSize || 0) > 1) {
            toast({
              title: `Связка: вещь ${order.groupPosition || 1} из ${order.groupSize}`,
              description:
                'Ярлык у заказа ОДИН — наклейте его на общую коробку. На саму вещь ' +
                'пойдёт стикер связки, он напечатается при закрытии заказа',
            });
          }
        } else {
