    ...
    {order.matchedByFallback && (
      <div className="flex items-start gap-3 rounded-md border-2 border-amber-500 bg-amber-50 p-4 text-amber-900">
        ...
        <div className="min-w-0">
          <p className="text-2xl font-bold">Номер заказа отличается от бумажки</p>
          <p className="text-lg">
            Отсканировано: <b>{order.scannedCode}</b>, найден заказ{' '}
            <b>{order.orderNumber}</b>. Так бывает, когда маркетплейс переименовал
            отправление. Сверьте размер и материал ниже — если не совпадают, нажмите
            «Отмена» и позовите старшего
          </p>
        </div>
      </div>
    )}
