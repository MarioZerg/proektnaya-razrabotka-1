...
const FbsSupplyChecklist = ({
  ...
  // ПОСЛЕДНЯЯ ОТСКАНИРОВАННАЯ ВЕЩЬ — В САМЫЙ ВЕРХ.
  //
  // Кладовщик пикает ярлык и должен сразу увидеть, ЧТО именно он положил в короб:
  // тот ли размер, тот ли материал. Раньше строка уезжала в общий список зелёных
  // и терялась среди сотни таких же — проверить себя было нельзя, и ошибку
  // замечали уже на складе площадки.
  //
  // Вещи в поставку добавляются по порядку, поэтому последняя — с наибольшим
  // номером записи.
  const lastScanned = scannedRows.reduce<Row | undefined>(
    (best, r) => (!best || (r.item?.id || 0) > (best.item?.id || 0) ? r : best),
    undefined,
  );

  // Дальше ненайденные: это и есть работа. Собранное кладовщик уже держит в коробе
  ...
  const flatRows = [
    ...(lastScanned ? [{ ...lastScanned, justScanned: true }] : []),
    ...waitingRows,
    ...scannedRows.filter((r) => r.key !== lastScanned?.key),
  ];
