 ...
 *
 * Это ПЕРЕпечатка для живой ситуации на складе: пакет порвался, кладовщик переупаковывает
 * вещь в новый — а ярлык остался на старом пакете. Поэтому ярлык доступен на всём отрезке,
 * пока вещь закреплена за отправлением и ещё не уехала: её подобрали под заказ («На
 * сборке»), зарезервировали («В резерве») или уже собрали («На поставку»).
 *
 * Ярлык привязан к конкретному отправлению, а не к бумажке: сколько раз его ни печатай,
 * код на нём один и тот же. Наклеить его можно только на ту вещь, за которой закреплён
 * заказ, — а её номер и заказ кладовщик видит в этой же строке.
 *
...
export const canPrintMarketplaceLabel = (item: GoodsWarehouseItem): boolean =>
  // Заказ у вещи бывает двух видов: подобранный с полки под новое отправление
  // (reservedOrderId) и собственный — когда вещь сшили сразу под этот заказ. Ярлык
  // нужен и там, и там, поэтому годится любой из них.
  Boolean(item.reservedOrderId || item.orderId) &&
