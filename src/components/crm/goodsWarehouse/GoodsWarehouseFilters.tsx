...
const GoodsWarehouseFilters = ({
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Отгруженных вещей десятки тысяч — весь список не показать. Открывают этот
          фильтр всегда по свежему случаю: вещь числится уехавшей, а лежит на столе.
          Поэтому три дня, а про поиск говорим прямо — иначе кладовщик решит, что
          вещь потерялась. */}
      {!search.trim() && statusFilter === 'shipped' && (
        <p className="text-xs text-muted-foreground">
          Показаны отгрузки за последние 3 дня. Вещь постарше — найдите поиском по
          стикеру хранения
        </p>
      )}
      ...
      <div className="min-w-0">
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent>
            ...
            {/* Отгруженные вещи. Раньше этого пункта не было, и найти такую вещь
                в таблице было нельзя вообще — только точным поиском по стикеру.
                А искать приходится: вещь могли вынуть из короба, вернуть с приёмки
