...
const PARTS: { key: string; label: string; hint?: string; color: string }[] = [
  ...
  { key: 'production', label: 'Себестоимость', color: 'bg-amber-500' },
  {
    ...
    // Это и есть ДРР — доля рекламных расходов в обороте. Процент рядом со
    // строкой и есть он: отдельно подписывать не нужно, но назвать привычным
    // словом стоит, иначе кажется, что это какая-то другая цифра.
    hint: 'ДРР — сколько кабинет списал за период',
    color: 'bg-orange-400',
  },
  {
    key: 'returns',
    label: 'Обработка возвратов',
    // Ткань и работу швеи сюда НЕ включаем: вещь вернулась на склад и будет
    // продана снова. Платим только упаковщице за разбор и переупаковку.
    hint: 'переупаковка вернувшихся вещей',
    color: 'bg-red-700',
  },
