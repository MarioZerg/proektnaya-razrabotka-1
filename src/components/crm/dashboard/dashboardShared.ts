...
export interface DashboardWidgetData {
  ...
  hint?: string;
  /**
   * С какого числа плитка начинает ПУЛЬСИРОВАТЬ красным вместо спокойного перелива.
   *
   * Плитки склада — это очередь работы, и она копится в течение дня. Пока в ней
   * немного, плитка просто переливается: работа есть, но не горит. Когда очередь
   * переваливает через порог, разбирать её нужно уже сейчас — плитка начинает
   * пульсировать и цепляет взгляд через весь экран.
   *
   * Порог у каждой плитки свой: пять невывезенных из цеха вещей — это уже завал,
   * а двадцать заканчивающихся рулонов — обычный рабочий объём. Значение 1
   * означает «пульсировать сразу, как только появилось» — так помечены возвраты
   * с ПВЗ: пока их не разобрали, товар не считается проверенным и не идёт в подбор.
   *
   * Без этого поля плитка не переливается и не пульсирует вовсе — так ведут себя
   * все показатели производства, они справочные, а не очередь задач.
   */
  pulseFrom?: number;
}
