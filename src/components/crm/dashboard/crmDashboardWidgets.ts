...
export const buildDashboardWidgets = ({
  ...
  if (canSeeWarehouseWidgets) {
    ...
    list.splice(4, 0, {
      ...
      // До пяти вещей кладовщик уносит за один заход — плитка спокойно
      // переливается. Шесть и больше — это уже отдельная ходка, и вещи
      // мешают в цехе: пульсируем.
      pulseFrom: 6,
    });
    list.splice(5, 0, {
      ...
      // Десяток сборок — обычный ход дня. Больше — очередь растёт быстрее,
      // чем разбирается, и отгрузка начинает опаздывать.
      pulseFrom: 11,
    });
    ...
    list.push({
      ...
      // Пульсируем с первой же вещи: пока возврат не осмотрен, он не лежит
      // на полке и не идёт в подбор — товар выпадает из оборота.
      pulseFrom: 1,
    });
    ...
    list.push({
      ...
      // Пара заканчивающихся рулонов — рабочая текучка. За двадцатью уже не
      // уследить на глазок, и цех рискует встать без ткани.
      pulseFrom: 21,
    });
  }
