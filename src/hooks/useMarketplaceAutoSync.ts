 ...
 *  - новые FBS-заказы (WildBerries и OZON) — раз в 30 минут;
 *  - заявки на возврат — раз в час (появляются реже, чаще опрашивать незачем).
 *
 * Это дополнение, а не основной способ: заказы тянет внешний планировщик по расписанию,
 * поэтому они приезжают и ночью, и в выходные, когда CRM никто не открывал. Здесь же —
 * чтобы админ во время работы видел свежие заказы, не дожидаясь следующего запуска.
 *
