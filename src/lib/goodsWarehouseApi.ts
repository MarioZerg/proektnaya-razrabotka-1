...
export type GoodsStatus =
  ...
  | 'to_dispose'
  /** Распущен в материал: метры легли на рулон, вещи как товара больше нет. */
  | 'returned_to_roll'
  | 'in_stock'
  | 'picking'
...
export interface GoodsWarehouseItem {
  ...
  /**
   * Номер этой поставки глазами человека: «2000065880431», «WB-GI-281233116».
   *
   * Кладовщику нужен не id из базы, а тот номер, которым поставка называется в
   * кабинете маркетплейса и на коробе. Без него найти отгрузку в списке можно
   * было только перебором.
   */
  supplyNumber?: string | null;
  /** Схема поставки (FBS/FBO) — подписываем рядом с номером. */
...
/**
 * Вернуть лишнюю вещь FBO с подбора обратно в свободный остаток склада.
 *
 * Товар FBO обезличен: в короб уезжает та вещь, что под рукой, а «запасная»
 * того же размера остаётся закреплённой за строкой заявки, хотя заявка ею уже
 * закрыта. Такая вещь лежит с чужим ярлыком OZN, в подбор не идёт и в короб не
 * нужна. Здесь с неё снимаются бронь и ярлык — вещь снова обычный остаток.
 *
 * Ярлык отправления сбрасывается всегда, поэтому в ответе приходят данные для
 * печати стикера хранения: без него вещь уедет на полку неопознанной.
 * Доступно администратору и старшему кладовщику.
 */
export const releaseGoodsToShelf = (
  id: number,
  shelfId?: number | null,
  note?: string,
  actorId?: number,
  ...
  postAction({ action: 'release_to_shelf', id, shelfId, note, actorId, actorName }) as Promise<{
    success: true;
    storageBarcode: string | null;
    product: string | null;
    material: string | null;
    shelfName: string | null;
    /** Ярлык снят — на вещь нужно напечатать стикер хранения. */
    needsStorageLabel: boolean;
...
export interface PickingOrder {
  ...
  shopColor?: string | null;
  /**
   * Вещь собрана СВЕРХ плана заявки FBO — нести её в короб не нужно.
   *
   * Товар FBO обезличен: в короб уезжает та вещь, что под рукой, а «запасная»
   * того же размера остаётся закреплённой за строкой заявки, хотя заявка ею
   * уже закрыта коробами. Такие вещи показываются отдельно от настоящей
   * работы: их возвращают на полки, а не ищут по стеллажам.
   */
  extraForSupply?: boolean;
}
...
export interface GoodsCard {
  ...
  /** Номер поставки из кабинета маркетплейса — им она подписана на коробе. */
  supplyNumber?: string | null;
