...
/**
 * Чего не хватает для печати листов. Пустой список — можно печатать.
 *
 * ЗАЧЕМ ЭТО НУЖНО. Лист клеится на короб и едет к перевозчику. Незаполненное поле
 * печатается прочерком, а в QR уходят нули — и это замечают уже на приёмке, когда
 * короба сняты с машины и переклеить их некому. Проверка стоит ДО печати: дешевле
 * не дать напечатать, чем разбирать непринятую поставку.
 *
 * Проверяем ровно то, что реально попадает на лист: заявку, обе даты, склад,
 * номер поставки, реквизиты клиента и код IDS. Остальное (IDO, номер заявки,
 * паллеты, забор) приходит из API Газельки и пустым не бывает.
 */
export const missingLabelFields = (data: PackingLabelData): string[] => {
  const { plan, supply } = data;
  const missing: string[] = [];

  if (!resolveShipDate(data)) missing.push('Дата отгрузки со склада');
  if (!plan.deliveryDate) missing.push('Дата поставки на склад маркетплейса');
  if (!plan.deliveryAddress) missing.push('Склад поставки');
  if (!supply.supplyNumber) missing.push('Номер поставки на маркетплейсе');
  // IDS — код склада Газельки, единственное, чего нет в их API. Без него QR
  // собирается с нулями, и короб не привязывается к складу перевозчика.
  if (!supply.gazelkaIds) missing.push('Код склада для QR (IDS)');
  // Клиент и телефон — общие настройки, а не поля поставки. Их заполняют один раз,
  // но пока они пусты, на каждом листе стоит прочерк: перевозчику не к кому
  // обратиться по грузу.
  if (!supply.gazelkaClientName?.trim()) missing.push('Клиент (в настройках Газельки)');
  if (!supply.gazelkaClientPhone?.trim()) missing.push('Телефон клиента (в настройках Газельки)');
...
export const printGazelkaLabels = async (data: PackingLabelData): Promise<void> => {
  ...
  // ПОСЛЕДНИЙ РУБЕЖ ПЕРЕД ПРИНТЕРОМ.
  //
  // Кнопки на экранах уже блокируются, но проверку дублируем здесь: экранов с
  // печатью два (карточка поставки и экран сборки), и любой новый вызов иначе
  // снова сможет отправить на принтер лист с прочерками. Ошибку показывает
  // вызывающий код — он знает, кому и где её вывести.
  const missing = missingLabelFields(data);
  if (missing.length) {
    throw new Error(
      `Не заполнено для печати: ${missing.join(', ')}. `
      + 'Лист с пустыми полями перевозчик не примет.',
    );
  }
