...
/**
 * Строка разбивки задания: конкретная позиция и сколько её штук нужно.
 *
 * Вещи одного размера на полке не отличаются — кладовщик берёт любую подходящую.
 * Поэтому важно не общее число, а «чего и по сколько»: иначе он сканирует всё
 * подряд, чтобы понять, что из списка ещё не собрано.
 */
export interface StorekeeperTaskItem {
  /** Название позиции, например «Сетка 500x265». */
  name: string;
  /** Сколько штук этой позиции осталось собрать. */
  qty: number;
}
...
export interface StorekeeperTask {
  ...
  adminClosed?: boolean;
  /**
   * Разбивка «что и сколько штук»: у подбора вещи одного размера на полке
   * одинаковы, и без этой расшифровки кладовщик пикал всё подряд, чтобы найти
   * нужные. Есть не у всех заданий — только там, где счёт идёт по позициям.
   */
  items?: StorekeeperTaskItem[];
}
