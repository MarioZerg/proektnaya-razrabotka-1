...
export const moveRobotPricesAll = async (
  ...
  // шага (25, потом 50, потом 75...). Складывать эти ответы нельзя — так
  // получается сумма ряда: на 674 карточках выходило «изменено 4634».
  // Просто берём последнее значение, оно и есть итог.
  //
  // Предохранитель от бесконечного круга: пачек хватит с запасом даже на
  // самый большой ассортимент, а зациклиться на ошибке сервера нельзя.
  let stumbles = 0;
  for (let guard = 0; last.inProgress && last.left && guard < 400; guard++) {
    onProgress?.(last.pushed || 0, last.left);

    // ОДНА НЕУДАЧНАЯ ПАЧКА НЕ ДОЛЖНА РОНЯТЬ ВЕСЬ ПРОГОН.
    //
    // Ozon держит ограничение на частоту обращений и время от времени
    // отвечает отказом, а сам вызов может оборваться по времени. Раньше
    // любая такая осечка выбрасывала ошибку наружу, и продвижение
    // останавливалось на середине ассортимента — с частью карточек по
    // новой цене и частью по старой.
    //
    // Теперь пропускаем удар: ждём и пробуем снова. Сервер вычёркивает
    // отправленное сам, поэтому повторный вызов продолжает с того же
    // места и ничего не двигает дважды. Сдаёмся только если осечки
    // идут подряд — значит, дело не в случайности.
    try {
