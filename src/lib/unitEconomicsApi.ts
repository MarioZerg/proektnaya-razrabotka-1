...
export interface UnitCalc {
  ...
  logistics: number;
  /**
   * Логистика внутри ставки удержания площадки.
   *
   * У OZON доставка и обработка возвратов зашиты в комиссию: цена минус
   * удержание в точности равны сумме к выплате. Вычитать их ещё раз
   * отдельной строкой значит задваивать расход.
   */
...
export interface HeightRow {
  ...
  /**
   * Откуда взята цена расчёта.
   *
   * realization — цена продавца из официального отчёта OZON: полная сумма
   * покупателя, включая оплаченную баллами. Она же налоговая база.
   * fact — начисление по финансовым операциям, showcase — витрина,
   * card — цена карточки без скидок.
   */
  priceSource2?: 'realization' | 'fact' | 'showcase' | 'card';
  /** Сколько продаж в отчёте о реализации. */
