 ...
 * штрихкоде, приёмка оформлена дважды. Со складских списков он исчезает, но
 * в приёмке строка остаётся с пометкой «убран»: приёмка — первичный документ,
 * и вычёркивать из него позицию задним числом нельзя.
 *
 * Рулон, по которому уже кроили заказы, убрать не получится — за ним стоит
 * выполненная работа.
 */
export const deleteRoll = (id: number, reason: string) =>
  postAction({ action: 'delete', id, reason });

/**
 * Правка метража рулона (только администратор).
 *
 * Бирки поставщика врут: на рулоне «50 м», по факту 47. Пока рулон целый лежит
 * на складе, цифру нужно поправить — иначе от неверного числа считается и
 * остаток материала, и себестоимость метра. Метраж меняется сразу и в рулоне,
 * и в приёмке, а себестоимость по всей машине пересчитывается.
 */
export const setRollQuantity = (id: number, quantity: number) =>
  postAction({ action: 'set_quantity', id, quantity }) as Promise<{
    success: true;
    quantity: number;
  }>;
