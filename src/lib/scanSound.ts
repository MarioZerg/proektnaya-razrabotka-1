...
/**
 * Заказ ОТМЕНЁН покупателем: вещь едет не в короб, а на полку хранения.
 *
 * Звучит иначе, чем обычная ошибка: три нисходящих сигнала, «отбой». Кладовщик
 * сканирует поставку подряд и на слух отличает «этой вещи тут не место» от
 * «не нашёл» — не вчитываясь в экран, он сразу откладывает вещь в сторону.
 */
export const playCancelSound = () => {
  play('/sounds/scan-cancel.mp3', () => {
    beep(520, 0.16, 'square');
    window.setTimeout(() => beep(400, 0.16, 'square'), 170);
    window.setTimeout(() => beep(280, 0.32, 'square'), 340);
  });
};
