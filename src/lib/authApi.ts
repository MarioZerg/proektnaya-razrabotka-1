...
export interface EnterRoleResult {
  ...
  shiftNumber: number | null;
  /**
   * Ключ сессии. По нему сервер понимает, кто делает запрос и что ему позволено.
   *
   * Раньше исполнителя брали из тела запроса, и сотрудник мог назваться
   * администратором прямо из консоли браузера. Теперь права считаются только
   * по этому ключу — подделать его нельзя.
   */
  token?: string;
}
