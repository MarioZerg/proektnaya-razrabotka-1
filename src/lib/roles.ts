...
const productionNav: NavItem[] = [
  ...
  { label: 'Магазин вариков', icon: 'Gift', path: '/crm/variki/shop' },
  {
    ...
    children: [
      ...
      // Куски на перешив — остатки ткани в цехе. Закройщику нужен как
      // справочник: что есть, прежде чем распускать новый рулон.
      { label: 'Куски на перешив', path: '/crm/inventory/repair-fabric' },
    ],
  },
  {
    label: 'Отгрузки',
...
const managerNav: NavItem[] = [
  ...
  // Склад товара менеджеру нужен ровно для одного: собрать товарный состав будущей
  // FBO-поставки и выгрузить его в Excel для площадки. Поэтому открыта ТОЛЬКО вкладка
  // «На хранении» — свободный остаток на полках. Складские операции (полки, приёмка,
  // возвраты, утилизация) остаются кладовщику: менеджер вещей в руках не держит.
  {
    label: 'Инвентаризация',
    icon: 'Boxes',
    children: [{ label: 'Склад товара', path: '/crm/inventory/goods-warehouse' }],
  },
  {
    label: 'Маркетплейсы',
    icon: 'ShoppingBag',
...
const adminNav: NavItem[] = [
  ...
  { label: 'Договоры', icon: 'FileSignature', path: '/crm/contracts' },
  {
    ...
    children: [
      ...
      // Продвижение — ручной подъём цен: весь ассортимент или выбранная
      // ткань и ширина. Автоматики и советов по товарам больше нет.
