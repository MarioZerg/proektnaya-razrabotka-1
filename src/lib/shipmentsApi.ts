...
export interface Shipment {
  ...
  /**
   * Стоимость перевозки. Ноль — сумму ещё не проставили.
   *
   * Счёт за машину часто приходит позже самой машины, поэтому в списке видно,
...
export interface ApproveSupplyResult {
  success: true;
  /**
   * Штрихкоды принятых рулонов.
   *
   * Подтверждение работает по правилу «всё или ничего»: если хоть одна позиция
   * не проходит, на склад не встаёт ничего и возвращается ошибка с номером
   * строки. Частичного прихода здесь быть не может — иначе кладовщик увидит
   * «принято», а часть рулонов до склада не доедет.
   */
  createdRolls: string[];
}
...
/**
 * Логистика принятой приёмки: дозаполнить или исправить (только администратор).
 *
 * Счёт за перевозку часто приходит позже машины, поэтому сумму дописывают потом —
 * она делится поровну на все метры приёмки и входит в себестоимость.
 *
 * Исправить уже указанную сумму можно, только пока весь материал приёмки цел на
 * складе: тронутый рулон унёс свою себестоимость в раскрои, списания и зарплату.
 */
